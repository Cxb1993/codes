...
def cavityPlotterY(ny, velScheme, color, output=""):
    ...
    if output == "show":
        ...
        plt.savefig(oFPath + "ny=" + str(ny) + "_cavityFlowY" + oFExt)
...
def cavityPlotterX(nx, velScheme, color, output=""):
    ...
    xexact = [0.0, 0.0547, 0.0625, 0.0703, 0.1016, 0.1719, 0.2813, 0.4531,
              0.5, 0.6172, 0.7344, 0.8516, 0.9531, 0.9609, 0.9688, 0.9766, 1.0]
    vexact = [0.0, -0.03717, -0.04192, -0.04775, -0.06434, -0.10150, -0.15662,
              -0.21090, -0.20581, -0.13641, 0.00332, 0.23151, 0.68717, 0.73722,
              0.78871, 0.84123, 1.0]
    plt.plot(xexact, vexact, ".")

    plotLabeller("Comparision of velocity profiles of Ghia et. al solution\
                 \nversus numerical schemes", "\(X\) (m)",
                 "\(v\)-velocity (m/s)")

    if output == "show":
        plt.show()
    else:
        plt.savefig(oFPath + "ny=" + str(ny) + "_cavityFlowX" + oFExt)
