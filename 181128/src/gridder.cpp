...
void volumeOfSolid()
{
    ...
    for (int i = 2; i <= nx-3; ++i)
    {
        ...
        double Xmid = ( X(i) + X(i+1) ) / 2.0;
        for (int j = 2; j <= ny-3; ++j)
        {
            double Ymid = ( Y(j) + Y(j+1) ) / 2.0;
            for (int k = 2; k <= nz-3; ++k)
            {
                double Zmid = ( Z(k) + Z(k+1) ) / 2.0;
                ...
                {
                    // Dxc, Dyx, Dzc are cell widths
                    double Dxc = Dx(i) / nSubGrids;
                    double Dyc = Dy(j) / nSubGrids;
                    double Dzc = Dz(k) / nSubGrids;

                    // Xg0, Yg0, Zg0 are midpoints of first subgrid cell
                    double Xg0 = X(i) + Dxc / 2.0;
                    double Yg0 = Y(j) + Dyc / 2.0;
                    double Zg0 = Z(k) + Dzc / 2.0;
                    ...
                    for (int l = 0; l < nSubGrids; ++l)
                    {
                        ...
                        double Xgmid = Xg0 + (Dxc * l);
                        for (int m = 0; m < nSubGrids; ++m)
                        {
                            double Ygmid = Yg0 + (Dyc * m);
                            for (int n = 0; n < nSubGrids; ++n)
                            {
                                double Zgmid = Zg0 + (Dzc * n);
