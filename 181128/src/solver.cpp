...
Matrix<double,3> FX (nx,ny,nz);
Matrix<double,3> FY (nx,ny,nz);
Matrix<double,3> FZ (nx,ny,nz);
...
void momentum()
{
    #pragma omp parallel for\
        default(none)\
        shared(FU, FV, FW, U, V, W, Dx, Dxs, Dy, Dys, Dz, Dzs)
...
void timeStepNSchemer()
{
    ...
    #pragma omp parallel for\
        default(none)\
        shared(U, V, W, FU, FU1, FU2, FV, FV1, FV2, FW, FW1, FW2)
    for (int i = 2; i <= nx-3; ++i)
    {
        for (int j = 2; j <= ny-3; ++j)
        {
...
void pressure()
{
    ...
    while (h <= maxPrIters && pChangeMax > pResidual)
    {
        ...
        #pragma omp parallel for\
            default(none)\
            shared(P, U, V, W, Dx, Dxs, Dy, Dys, Dz, Dzs)\
...
void velUpdater()
{
    #pragma omp parallel for\
        default(none)\
        shared(U, V, W, Uo, Vo, Wo, FX, FY, FZ, P, ETA, Dxs, Dys, Dzs)\
        reduction(max:uChangeMax, vChangeMax, wChangeMax)
    for (int i = 2; i <= nx-3; ++i)
    {
        for (int j = 2; j <= ny-3; ++j)
        {
            for (int k = 2; k <= nz-3; ++k)
            {
                // Updating velocities with pressure
                U(i,j,k) = U(i,j,k) - dt * ( P(i+1,j,k) - P(i,j,k) ) / Dxs(i);
                V(i,j,k) = V(i,j,k) - dt * ( P(i,j+1,k) - P(i,j,k) ) / Dys(j);
                W(i,j,k) = W(i,j,k) - dt * ( P(i,j,k+1) - P(i,j,k) ) / Dzs(k);

                // Calculating velocities for solid (or partially solid) cells
                double uNew, vNew, wNew;
                uNew = ETA(i,j,k) * uS + (1 - ETA(i,j,k)) * U(i,j,k);
                vNew = ETA(i,j,k) * vS + (1 - ETA(i,j,k)) * V(i,j,k);
                wNew = ETA(i,j,k) * wS + (1 - ETA(i,j,k)) * W(i,j,k);

                // Calculating virtual force
                FX(i,j,k) = (uNew - U(i,j,k)) / (dt * ETA(i,j,k) + 1.0e-12);
                FY(i,j,k) = (vNew - V(i,j,k)) / (dt * ETA(i,j,k) + 1.0e-12);
                FZ(i,j,k) = (wNew - W(i,j,k)) / (dt * ETA(i,j,k) + 1.0e-12);

                // Updating velocity vectors with new values
                U(i,j,k) = uNew;
                V(i,j,k) = vNew;
                W(i,j,k) = wNew;

                // Checking for the maximum velocities
                ...
                // Updating old velocity vectors with new values
                Uo(i,j,k) = U(i,j,k);
                Vo(i,j,k) = V(i,j,k);
                Wo(i,j,k) = W(i,j,k);
            }
...
void virtualForceIntegrator()
{
    Matrix<double,1> FXY(nx);
    Matrix<double,2> FXZ(nx,ny);
    Matrix<double,1> FYY(nx);
    Matrix<double,2> FYZ(nx,ny);
    Matrix<double,1> FZY(nx);
    Matrix<double,2> FZZ(nx,ny);
