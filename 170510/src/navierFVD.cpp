...
void pressure()
{
    ...
    while (m <= maxPressIterations && abs(maxChange) > pressResidual)
    {
        maxChange = 0.0;
        for (int i = 1; i < nx-1; ++i)
        {
            for (int j = 1; j < ny-1; ++j)
            {
                ...
                if ((data - P[i][j]) > maxChange)
                {
                    maxChange = data - P[i][j];
                }

                // Applying the SOR method through following formula:
                P[i][j] = P[i][j] + omega * (data - P[i][j]);
                // P[i][j] = (1.0 - omega)*P[i][j] + omega * data; //same
            }
        }
        pressBoundary();
        cout << m++ << '\t';
...
void velUpdater()
{
    for (int i = 1; i < nx-1; ++i)
    {
        for (int j = 1; j < ny-1; ++j)
        {
            U[i][j] -= dt/rho * (P[i+1][j] - P[i][j])/dx;
            V[i][j] -= dt/rho * (P[i][j+1] - P[i][j])/dy;
        }
    }
}
