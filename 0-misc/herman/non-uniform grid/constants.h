...
const int y_z_ratio = 1;
const double ly = 21.0;
const double lx = 7.0;//ly * static_cast<double>(x_y_ratio);
const double lz = 7.0;//ly * static_cast<double>(y_z_ratio);
// Dimensions
const double r = 0.5;
const double L = lx;
const double B = ly;
...
const int n_s = 15 * 4 * r; //number of grid in solid body area
const int nx_l = 8 * (L -(4 * r)); //number of grid in solid body area
const int ny_l = 8 * (B -(4 * r)); //number of grid in solid body area
const int nz_l = 8 * (H -(4 * r)); //number of grid in solid body area

// Number of nodes on the x and y axes
// Compile-time constants needed for native C-style arrays
const int nxyz = ny_l + n_s;                    // grid dimension
const int ghostCells = 4;                  // total ghost cells in an axis
const int nx = (n_s+ nx_l) + ghostCells+1;
const int ny = (n_s+ ny_l) + ghostCells+1; //nx / x_y_ratio;
const int nz = (n_s+ nz_l) + ghostCells+1;



// Intial and/or boundary conditions
// const double uin = Uin;
const double uIn = 0.0;
...
const double vInitial = 1.0;
const double wInitial = 0.0;

// Properties
const double nu = 0.025;               // kinematic viscosity (hypothetical)
...
const double pResidual = 1.0e-5;        // residual criteria for pressure
...
extern Numeric_lib::Matrix<double,3> U_DS;
extern Numeric_lib::Matrix<double,3> V_DS;
extern Numeric_lib::Matrix<double,3> W_DS;
extern Numeric_lib::Matrix<double,3> VF_X;
extern Numeric_lib::Matrix<double,3> VF_Y;
extern Numeric_lib::Matrix<double,3> VF_Z;
extern Numeric_lib::Matrix<double,1> TF_X;
extern Numeric_lib::Matrix<double,1> TF_Y;
extern Numeric_lib::Matrix<double,2> F_X;
extern Numeric_lib::Matrix<double,2> F_Y;
extern Numeric_lib::Matrix<double,3> STR;
extern Numeric_lib::Matrix<double,3> VORT;
