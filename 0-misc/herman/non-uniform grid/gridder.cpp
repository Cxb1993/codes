...
void gridder()
{
    // Define the grid coordinates
        double ax = Xc - (2 * r)-0.1;
        double bx = Xc + (2 * r);

        double ay = Yc - (2 * r)-0.1;
        double by = Yc + (2 * r);

        double az = Zc - (2 * r)-0.1;
        double bz = Zc + (2 * r);
        ...
        X(1) = 0.0 - (L / static_cast<double>(8*L));
        Y(1) = 0.0 - (B / static_cast<double>(8*B));
        Z(1) = 0.0 - (H / static_cast<double>(8*H));

    for (int i = 2; i <= nx-2; ++i)
    {
         if ( Dis_x >= ax && Dis_x <= bx )
        {
            X(i)  = X(i-1) + (L / static_cast<double>(15*L));
            Dis_x = X(i) + (1/8);
        }
        else
        {
            X(i)  = X(i-1) + (L / static_cast<double>(8*L));
            Dis_x = X(i) + (1/8);
        }
    ...
    {
         if ( Dis_y >= ay && Dis_y <= by )
        {
            Y(j)  = Y(j-1) + (B / static_cast<double>(15*B));
            Dis_y = Y(j) + (1/8);
        }
        else
        {
            Y(j)  = Y(j-1) + (B / static_cast<double>(8*B));
            Dis_y = Y(j) + (1/8);
        }
    ...
    {
        if ( Dis_z >= az && Dis_z <= bz )
        {
            Z(k)  = Z(k-1) + (H / static_cast<double>(15*H));
            Dis_z = Z(k) + (1/8);
        }
        else
        {
            Z(k)  = Z(k-1) + (H / static_cast<double>(8*H));
            Dis_z = Z(k) + (1/8);
        }
        // // For finer grid intervals in increasing y direction:
        // Z(k) = H * sin( 0.5 * pi
