...
double X[2*nx-5] = {0.0};        // x coordinates
double Y[2*ny-5] = {0.0};        // y coordinates
...
void gridder()
{
    ...
    for (int j = 0; j < (2*ny-5); ++j)
    {
        // Y[j] = ly * (sin( pi * (j * 1.0 / (ny-1)) * 0.5));
        Y[j] = static_cast<double>(j) * dy / 2;
    }
    filer1(X, (2*nx-5), filePath + "coordinateX_ny-" + to_string(ny));
    filer1(Y, (2*ny-5), filePath + "coordinateY_ny-" + to_string(ny));
}
