...
void navierAnalytical()
{
    ...
    */
    int q = static_cast<int>(ceil(static_cast<double>(2*ny-5) / 2)) - 2;
    double y = 0.0;
    // cout.precision(numeric_limits<double>::digits10 + 2);
    cout << "j\t" << "q\t" << "y\t\t" << "y/D" << "\n";
    for (int j = 0; j < static_cast<int>(ceil(static_cast<double>(ny-3) / 2));
        ++j)
    {
        y = -Y[q];
    ...
    cout << "\nj\t" << "q\t" << "y\t\t" << "y/D" << "\n";
    q = 1;
    for (int j = static_cast<int>(ceil(static_cast<double>(ny-3) / 2));
        j < (ny-3); ++j)
    {
        y = Y[q];
        Ua[j] = uAvg * 1.5 * (1.0 - ((2.0 * y / D) * (2.0 * y / D)));
    ...
    filer1(Ua, (2*ny-5), velScheme + "_ny-" + to_string(ny) + "_exact_U");
}
