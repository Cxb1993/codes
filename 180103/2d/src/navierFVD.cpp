...
void momentum(string velocityScheme)
{
    ...
    for (int i = 1; i < nx-1; ++i)
    {
        for (int j = 1; j < ny-1; ++j)
        {
            ...
            un  = U[i][j]   + ( U[i][j+1]   - U[i][j]   ) / 2.0;
            us  = U[i][j-1] + ( U[i][j]     - U[i][j-1] ) / 2.0;
            uev = U[i][j]   + ( U[i][j+1]   - U[i][j]   ) / 2.0;
            uwv = U[i-1][j] + ( U[i-1][j+1] - U[i-1][j] ) / 2.0;
            ve  = V[i][j]   + ( V[i+1][j]   - V[i][j]   ) / 2.0;
            vw  = V[i-1][j] + ( V[i][j]     - V[i-1][j] ) / 2.0;
            vnu = V[i][j]   + ( V[i+1][j]   - V[i][j]   ) / 2.0;
            vsu = V[i][j-1] + ( V[i+1][j-1] - V[i][j-1] ) / 2.0;
