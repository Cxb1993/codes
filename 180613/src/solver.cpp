...
void velBoundary()
{
    ...
    // #pragma omp parallel for default(none) shared(U,V,W) private(i,j)
    for (i = 0; i <= nx-1; ++i)
    {
        for (j = 0; j <= ny-1; ++j)
        {
    ...
    // #pragma omp parallel for default(none) shared(U,V,W) private(j,k)
    for (j = 0; j <= ny-1; ++j)
    {
        for (k = 0; k <= nz-1; ++k)
        {
    ...
    // #pragma omp parallel for default(none) shared(U,V,W) private(i,k)
    for (i = 0; i <= nx-1; ++i)
    {
        for (k = 0; k <= nz-1; ++k)
        {
...
void pressBoundary()
{
    ...
    // #pragma omp parallel for default(none) shared(P) private(i,j)
    for (i = 1; i <= nx-2; ++i)
    {
        for (j = 1; j <= ny-2; ++j)
        {
    ...
    // #pragma omp parallel for default(none) shared(P) private(j,k)
    for (j = 1; j <= ny-2; ++j)
    {
        for (k = 1; k <= nz-2; ++k)
        {
    ...
    // #pragma omp parallel for default(none) shared(P) private(i,k)
    for (i = 1; i <= nx-2; ++i)
    {
        for (k = 1; k <= nz-2; ++k)
        {
...
void momentum(void (*velschemer)(int i, int j, int k))
{
    ...
    //     shared(FU, FV, FW, U, V, W, Dx, Dxs, Dy, Dys, Dz, Dzs, velschemer)\
    //     private(i, j, k,\
    //         ue, uw, un, us, uf, ub, vnu, vsu, wfu, wbu,\
    //         ve, vw, vn, vs, vf, vb, uev, uwv, wfv, wbv,\
    //         we, ww, wn, ws, wf, wb, uew, uww, vnw, vsw)*/
...
void timeStepNSchemer()
{
    ...
    /*// #pragma omp parallel for default(none)\
    //     shared(t, U, V, W, FU, FV, FW, FU1, FV1, FW1, FU2, FV2, FW2)\
    //     private(i,j,k)*/
    for (i = 2; i <= nx-3; ++i)
    {
        for (j = 2; j <= ny-3; ++j)
...
void pressure()
{
    ...
    while (h <= maxPressIters && pChangeMax > pResidual)
    {
        ...
        //     private(i,j,k, mChange, pChange, pNew)*/
        for (i = 2; i <= nx-3; ++i)
        {
            for (j = 2; j <= ny-3; ++j)
            {
...
void velUpdater()
{
    ...
    /*// #pragma omp parallel for default(none)\
    //     shared(P, U, V, W, Uo, Vo, Wo, Dxs, Dys, Dzs,\
    //         uChangeMax, uChangeMax_i, uChangeMax_j, uChangeMax_k,\
    //         vChangeMax, vChangeMax_i, vChangeMax_j, vChangeMax_k,\
    //         wChangeMax, wChangeMax_i, wChangeMax_j, wChangeMax_k)\
    //     private(i,j,k)*/
    for (i = 2; i <= nx-3; ++i)
    {
        for (j = 2; j <= ny-3; ++j)
