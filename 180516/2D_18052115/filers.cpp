...
{
    /*
    Function to file a 2D array. It can be used to map x, y and z direction
...
void filerAllSol(const int timeStep)
{
    ...
    if (timeStep == -1)
    {
        ...
        filer2DArray(U, Xa, Ys, nx-4, ny-5, ghostCells/2-1, ghostCells/2, "Vx",
                     fileUniqueName + "_" + to_string(nxyz) + "_U-final");
        filer2DArray(V, Xs, Ya, nx-5, ny-4, ghostCells/2, ghostCells/2-1, "Vy",
                     fileUniqueName + "_" + to_string(nxyz) + "_V-final");
        filer2DSolution(fileUniqueName + "_" + to_string(nxyz) + "_sol-final");
    }
    else
    {
